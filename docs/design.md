...
Suite count: 351 (348 fast + the 3 slow tests). Breakdown by area:
...
- **72 eval-suite unit tests** - metric primitives (percentile / p95, hit-rate@k, MRR, expected calibration error, conformance), per-model cost table, the scorecard generator, scorer (severity tolerance, CVE recall threshold, KEV / ransomware honoring), runner (SSE CRLF/LF tolerance, error-event surfacing, missing-final handling, HTTP 5xx, `usage`-event token capture), and the hard-mode keyword-query derivation for the retrieval eval.
