...
Suite count: 555 (552 fast + the 3 slow tests). Breakdown by area:
...
- **30 API tests** - opt-in auth + per-IP rate limit, model-override allowlist, SSE framing, deterministic SSVC + grounding stamped onto the final report (grounded / suspect / not_evaluated paths), `usage` event emission, audit integration (one event per call, success or error path), and the stateless export endpoints (SARIF / OpenVEX render, artifact-uri default, invalid-report and non-purl-product refusals, auth).
