...
sequenceDiagram
    ...
    Note over API: server stamps ssvc = assess_ssvc(cves, trajectory):<br/>deterministic Act / Attend / Track* / Track over KEV / EPSS / exploit /<br/>ransomware / CVSS read from the captured tool returns, never from the<br/>report's fields; basis evidence / mixed / report (agent/ssvc.py)
...
**Why the SSVC *verdict* is a deterministic post-processor even though the prose heuristic is not (S1).** The prior decision keeps the natural-language remediation in the LLM's hands. A prioritization *verdict* is different: it is a safety- and compliance-relevant classification, and a probabilistic system that could return a different verdict on two identical runs is the wrong tool for it. So `agent/ssvc.py` computes the SSVC decision (Act / Attend / Track* / Track) deterministically from the signals the tools returned during the run (KEV / EPSS / public-exploit / ransomware / CVSS, read from the captured trajectory through the same evidence index the grounding verifier uses; a signal with no usable tool evidence falls back to the report and is named on the verdict as unverified, so the stamp's `basis` says how much of it rests on tool returns), the API stamps it onto `TriageReport.ssvc` after the model returns, and the prompt tells the model to *echo* the decision in `recommended_action` prose (and to leave the field null). The two paths should agree; when they diverge, the structured field is authoritative. Honesty about scope: this is SSVC-*informed*, not the full CISA tree - the "Automatable" and "Mission & Well-being" decision points are approximated (EPSS as a likelihood proxy; deployment-specific asset criticality is out of scope for a stateless tool), and that limitation is stated in the module docstring and the verdict rationale rather than hidden.

**Why prompt caching is on, and why the cost estimate prices cache tokens.** Every round of the ReAct loop re-sends the whole conversation: system prompt, eleven tool schemas, the output schema, every earlier tool return. On the recorded golden runs 74% of the billed input was that re-sent prefix (log4shell: 102,652 input tokens billed for a final context of 26,170), and 9 of 11 runs also burned a full extra round on a `string_too_long` retry the prompt had never warned about. Two fixes with no behaviour change: the prompt now states the three character budgets the schema enforces, and the agent sets three Anthropic cache breakpoints (instructions, tool definitions, and the automatic one that follows the conversation), so round n reads rounds 1..n-1 from cache. Cache reads are billed at a tenth of the input price and writes at 1.25x, so `eval/cost.py` prices them separately and the usage event carries them: the denial-of-wallet window and the scorecard charge what is actually billed rather than the pre-caching worst case.
...
**Untrusted-content fencing test invariant.** Unit tests assert that fenced fields start and end with the marker tokens (which carry a per-process random id), that marker-shaped tokens inside a payload are neutralized, and that every fenced field's cap is its payload budget plus `FENCE_OVERHEAD` (`tests/mcp_server/test_security.py` pins the inventory against the security module's docstring). Renaming or removing the markers, or hand-typing a cap, will fail tests before it ships.
...
|---|---|---|
| `fence_untrusted(t)` wraps any non-empty `t` with both markers, exactly once | `test_invariants.py` | the boundary contract cannot drift silently |
| The text is preserved inside the fence with marker-shaped tokens neutralized (`<` escaped) | `test_invariants.py` | we sanitize the BOUNDARY, not the CONTENT (the LLM still needs the data), and a forged marker is content |
| With a budget, the fenced output never exceeds budget + `FENCE_OVERHEAD` | `test_invariants.py` | a marker-stuffed payload cannot overflow a field cap |
...
- **10 deployment and version tests** - the compose wiring the code assumes (`MCP_AUTH_TOKEN` on both services, the per-run bounds on `agent-api`), blank-means-default parsing of the bound settings, and the single-sourced package version.
