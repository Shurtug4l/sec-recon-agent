...
This is the design story behind one decision in sec-recon-agent: how an LLM agent
can ingest adversary-influenced vulnerability intelligence and still produce a
report you can act on. It is written for engineers and reviewers who want the
reasoning, not just the control list. The reference matrices live in
...
ATLAS AML.T0055). The dangerous input is not a side channel to be closed; it is
...
control*, and make the output *incapable of expressing a successful injection*.

## The design: four independent layers

Each layer assumes the one in front of it has failed. None of them is the single
point the security depends on.
...
This layer is deliberately treated as the weakest of the four. It raises the cost
...
`severity` is a four-value enum, `confidence` is a three-value enum, every CVE id
...
### Layer 4: falsifiable tests, not assertions of safety
...
- 8 prompt-injection payloads (system-prompt extraction, role-play, fake
  authority, Log4Shell-style `${jndi:...}`) are asserted to survive *inside* the
...
The red-team battery doubles as a regression detector: a system-prompt edit or a
model swap that quietly weakens resistance shows up as a dropped resistance rate,
and a drift test refuses to merge a new production payload that lacks an ATLAS
tag. Security that is not continuously re-falsified decays into folklore.
...
field; the reason that is acceptable here is that the schema is *not* removed, and
the schema is what bounds the damage. The audit trail (a SHA-256 hash chain over
each triage) is tamper-evident, not tamper-proof, and is demo-grade rather than a
production WORM store. These limits are written down in
...
function. You can make reading it grant no authority. Two moves do the work:
mark untrusted data so its trust status is a fact the code owns rather than a
judgment the model makes, and constrain the output to a schema so narrow that a
fully successful injection can only produce a wrong-but-valid answer your
evaluation already catches. The system prompt and the marker are the layers an
attacker probes; the structured output boundary is the one that holds.

That ordering, soft controls in front of a hard one, with falsifiable tests
behind all of them, is the part of this project meant to outlast the specific
domain.
