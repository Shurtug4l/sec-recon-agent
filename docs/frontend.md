...
│   ├── command-palette.tsx      # Cmd+K provider: keydown listener, commands + live docs global search, triage ctx
...
└── lib/
    ...
    ├── docs-generated.json      # GENERATED by scripts/gen-docs.mjs from the repo's markdown (do not hand-edit)
...
`scripts/gen-docs.mjs` (build-time, dev-only deps) turns the repo's markdown
into `lib/docs-generated.json`; see [In-app documentation](#in-app-documentation).
...
`Cmd+K` (macOS) / `Ctrl+K` (elsewhere), or the Search button in the header, opens a command palette with 61 commands in 7 groups: report actions (copy link, export .md / JSON / PDF, show grounding verification, show SSVC decision trace - visible only while a report is on screen, PDF / grounding / trace only where the report view is in the DOM; the trace command dispatches a window event so the closed-by-default disclosure opens and scrolls), page navigation (including Docs), dashboard tab jumps (including the audit trail), the 7 demo triage runs (live mode submits the same query to the real backend), the 12 guide sections, the guide's 23 external references, and project actions (GitHub, copy system prompt). Typing also runs a live global search over the docs corpus: matches surface as a **Documentation** group (`searchDocs`, the same engine the `/docs` page uses, lazy-imported on first open so the ~650 KB corpus stays code-split off the shared bundle), and picking one opens `/docs` at that section - so the palette is a single "search anything" surface, not a second engine. Doc hits are pre-filtered by `searchDocs`, so the palette passes cmdk a `filter` that keeps any `dochit:`-valued item verbatim (returning a positive score, so they still count toward `filtered.count` and the empty state stays correct) while commands keep cmdk's default fuzzy match. The static registry stays bounded because doc hits are rendered dynamically, not registered as commands. The `/docs` page keeps its own in-page search box for browsing sections while reading.
...
The `/docs` route renders the project's own markdown documentation inside the app, in the same design system, with cross-document search. The heavy lifting happens at **build time**, not in the browser: `frontend/scripts/gen-docs.mjs` turns each markdown file into `lib/docs-generated.json` through a `unified` pipeline (GFM tables, `rehype-slug` heading anchors, `rehype-highlight` syntax highlighting, a `rehype-sanitize` allowlist, plus local plugins that strip decorative images / badges, swap ` ```mermaid ` fences for `.docs-mermaid` placeholders, and wrap tables in horizontal-scroll containers). It also extracts a title, a purpose blurb, and a per-section index (heading id + prose) used by search. Every one of those dependencies is a **devDependency**, so the client ships no markdown parser and no highlighter - only the generated HTML.

**The corpus is assembled by discovery, not by an allowlist (P9).** Every tracked markdown file across `docs/`, the repo root, and `examples/` is a candidate; each candidate must be assigned to a group in `gen-docs.mjs` (with a short rail label) or listed in `EXCLUDE_BASENAMES`. A file that is present but unconfigured **fails the build**, so a new doc cannot be silently dropped and `examples/triage_walkthrough.md` is closed by construction rather than by memory. The two documented exclusions are `CLAUDE.md` (untracked, so present locally but absent in CI - ingesting it would flap the freshness gate) and `SCORECARD.md` (already the `/scorecard` route and a `make scorecard` artifact). The README is surfaced as the **Overview** and is the landing document (far lighter than `design.md`'s 72 KB first-paint); its shields.io badges and demo GIF are dropped in-app by the image strip but stay on the GitHub README. Each doc carries its repo-relative `path` (`README.md`, `docs/design.md`, `examples/triage_walkthrough.md`), shown in the panel header and used for the GitHub link.

The generator's output is a pure function of the corpus content **and** of the fixed group order (never of `readdir` order, which is filesystem-dependent), and it is committed: the frontend Docker build context is `frontend/` only, so `../docs` is absent there and the build reuses the committed JSON. Where `docs/` *is* present (local dev, CI, the Pages build) `prebuild` / `predev` / `build:demo` regenerate it, and a CI step (`ci-frontend.yml`) regenerates and `git diff --exit-code`s the JSON to fail on drift.
...
## Completeness gate (P12)

Three checks fail the build on the regressions the manual pre-push verification used to catch by hand. Two are browser-free and deterministic; the third is the Playwright + axe sweep, now a committed script instead of a scratchpad ritual. They run pre-merge in `.github/workflows/ci-frontend-e2e.yml`, which installs Playwright ephemerally (`--no-save`) so the browser never becomes a `package.json` dependency and the other frontend jobs stay lean.

- **Dead cross-link detection** (`gen-docs.mjs`, on the docs-freshness step). After the corpus renders, every P10-rewritten cross-link must resolve: the target slug is a real doc and, when the link carries a section anchor, the target doc renders an element with that id; a routed link points at a real App Router route (discovered from `src/app`). The validation data (anchor ids, link markers) is read post-sanitize and never persisted, so the generated JSON stays byte-identical. This is the full corpus check a browser cannot do - the `/docs` export server-renders only the default doc, so every other doc's cross-links live solely in the build-time HTML.
- **Unresolved-href detection** (`scripts/check-links.mjs`). A static, browser-free scan of the exported `out/`: every internal href resolves to an exported file, every `?doc=` deep-link names a real slug, and no absolute link escapes the Pages sub-path.
- **Accessibility + overflow sweep** (`scripts/a11y-sweep.mjs`). Serves the base-path export the way Pages does, then across every route in both themes asserts no horizontal overflow across a dense width band and no serious/critical axe violation at representative widths. Overflow is `scrollWidth > clientWidth` (scrollbar-independent, so macOS and Linux agree on the meaning) and a suspected overflow is re-measured after a settle window, so a transient Recharts resize flash is not mistaken for a real one.

The sweep paid for itself on its first run by catching two overflows that only classic-scrollbar platforms (Linux, Windows) render and macOS overlay scrollbars hide - so every prior macOS-only pass missed them:

- the header fit a 1024 px window with no margin (P6 tuned that fit on macOS); the search button's label and `Cmd+K` hint pushed the bar ~18 px past a 1024 px window, so both defer to `xl` now and the search is icon-only across the `lg`-`xl` band;
- the dashboard severity chart flashed the Recharts default width on a 360 px phone because the grid cell's default `min-width:auto` let the flash size the cell - fixed with `min-w-0` on the grid items so the cell shrinks to its track.
...
| Command palette doubles as global docs search (P11) | The header already exposes the palette as a visible search box, but it only navigated - a reader searching "SSVC" got the command, never the passage. Folding the existing `searchDocs` into the palette as a lazy-loaded **Documentation** group makes `Cmd+K` a single "search anything" surface and closes the real gap (there was no global content search), reusing one engine rather than adding a second. Lazy `import()` keeps the ~650 KB corpus code-split off every non-`/docs` route; a `filter` that whitelists `dochit:`-valued items lets cmdk keep the pre-filtered hits without re-scoring them, and doc hits stay dynamic so the static command count is untouched | A separate search route or modal (a second engine and a second mental model); registering every doc section as a static command (hundreds of registry entries + the 650 KB corpus in the shared bundle); static-importing `searchDocs` into the globally-mounted palette (drags the corpus into every route); leaving the two searches split (the palette stays navigation-only, the content gap persists) |
...
| Docs corpus by discovery, unconfigured = build failure (P9) | The old allowlist silently skipped anything unlisted, so `examples/triage_walkthrough.md` sat orphaned and any new doc could vanish unnoticed. Discovery across `docs/` + root + `examples/` with a fail-on-unconfigured gate makes the corpus self-maintaining: a present doc must be grouped or explicitly excluded, closing orphans by construction. Output stays driven by the fixed group order (not `readdir`) so the freshness diff is byte-stable across machines | Keeping the curated allowlist (the orphan and the silent-drop failure mode were its direct cost); auto-grouping every discovered file (loses the deliberate rail grouping and short labels); discovering at runtime (would ship a parser and lose the build-time freshness guarantee) |
| README surfaced as the Overview + `/docs` landing (P9) | `design.md` (72 KB) was the heaviest first-paint as the default doc; the README is the natural orientation document and far lighter, so it becomes the Overview at the top of the rail and the default. Its badges and demo GIF are stripped in-app (no doc uses raster images for content) but kept on GitHub, where the CI/license badges earn their place on the repo landing | Editing the README source to drop badges/GIF (degrades the GitHub landing for an in-app concern); a bespoke short intro doc (a second thing to keep in sync with the README); leaving `design.md` as the default (the 72 KB first-paint the move exists to fix) |
| Completeness gate: dead cross-link + unresolved href + a11y/overflow sweep (P12) | The clarity program added cross-links (P10) and a wider corpus (P9) that no check validated, and the responsive verification was a macOS-only manual ritual that let two classic-scrollbar overflows ship. Two browser-free gates (corpus cross-links in `gen-docs`, exported hrefs in `check-links`) plus a committed Playwright/axe sweep make all three blocking pre-merge | Trusting the manual pre-push pass (exactly what let the P6-class header overflow and the 360 px chart flash reach the live site); a DOM-based cross-link check (the export server-renders only the default doc, so it cannot see the other docs' links - build time is the only place the whole graph exists) |
| Playwright installed ephemerally in CI, not a `package.json` dep (P12) | The browser binary is heavy and only one job needs it; a `--no-save` install in `ci-frontend-e2e.yml` keeps `npm ci` in the type-check/build and Pages-deploy jobs lean and leaves the lockfile untouched, while the sweep script stays committed and runnable locally | Declaring `playwright` + `@axe-core/playwright` as devDependencies (drags the install into every job and churns the lockfile); keeping the sweep in the session scratchpad (the ritual that let the overflows slip) |
