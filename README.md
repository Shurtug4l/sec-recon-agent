...
The backend CI runs on a Python version matrix (3.12 + 3.13) so the declared `requires-python = ">=3.12"` is actually exercised, not just declared. Coverage on the fast suite holds at **~89%** with a soft 70% floor (`tool.coverage.report.fail_under`).

**Suite count: 306 passing** (304 fast + 2 slow ChromaDB round-trip, excluded from the fast run). Breakdown by area:
...
- **58 eval-suite unit tests** — metric primitives (percentile / p95, hit-rate@k, mean reciprocal rank, expected calibration error, structured-output conformance), per-model cost table, the scorecard generator (deterministic coverage + eval / retrieval / red-team JSON parsers + markdown builder + deterministic-mode CLI), scorer (severity tolerance, CVE recall threshold, KEV / ransomware honoring), and runner (SSE CRLF/LF tolerance, error-event surfacing, missing-final handling, HTTP 5xx, `usage`-event token capture).
